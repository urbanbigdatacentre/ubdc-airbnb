...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser):
        ...
        parser_get_details: ArgumentParser = subparsers.add_parser(
            "listing-detail", help="get the listing Details for a listing"
        )
        parser_get_booking_quote: ArgumentParser = subparsers.add_parser(
            "booking-quote", help="get a Booking Quote for a listing"
        )
        parser_get_user: ArgumentParser = subparsers.add_parser(
            "user-detail",
            help="get the User",
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if options.get("command") == "listing-detail":
                ...
                response = (
                    AirBnBListing.responses.objects.filter(
                        listing_id=listing_id, _type=AirBnBResponseTypes.listingDetail
                    )
                    .order_by("timestamp")
                    .first()
                )
            ...
            if options.get("command") == "booking-quote":
                ...
                booking_response = (
                    listing_obj.responses.filter(_type=AirBnBResponseTypes.bookingQuote).order_by("timestamp").first()
                )

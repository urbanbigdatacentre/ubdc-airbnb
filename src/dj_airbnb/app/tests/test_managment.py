...
from app.models import AOIShape, WorldShape
...
class ManagmentCmdsTest(TransactionTestCase):
    ...
    def test_add_aoi(self):
        out = StringIO()
        call_command('add_aoi', create_grid=False, geo_file=self.test_input_aoi.as_posix(), stdout=out)
    ...
    def test_add_mask_aoi_grids(self):
        ...
        call_command('add_aoi', create_grid=True, geo_file=self.test_input_aoi.as_posix(), stdout=out)
        assert AOIShape.objects.all().count() >= 1
